...
async function main() {
  ...
  clipCheck.onchange = function () {
    if (clipCheck.checked) {
      clipDepthSlider.style.display = 'inline-block';
      clipDepthSlider.oninput();
    } else {
      clipDepthSlider.style.display = 'none';
      nv1.setClipPlane([2, 0, 90]);
    }
  };
  clipDepthSlider.oninput = function () {
    if (clipCheck.checked) {
      const depth = parseInt(clipDepthSlider.value) / 100.0;
      nv1.setClipPlane([depth, 0, 90]); // depth, azimuth, elevation
    }
  };
